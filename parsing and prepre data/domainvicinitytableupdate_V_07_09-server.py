...
class WikiErrorCorrection:
    ...
    def make_list_from_sorted_json(self, revision_data_folder):
        ...
        modified_data_path=os.path.join('datasets','Table_07_09_for_creating_model')
                            ...
                            for nested_nested_folder in os.listdir(os.path.join(rdd_folder_path,nested_folder)):
                                ...
                                if os.path.isdir(os.path.join(page_folder, nested_nested_folder)):
                                    ...
                                    if revision_list:
                                        ...
                                        json.dump(revision_list, open(os.path.join(modified_data_path, nested_folder + '_' + str(table_No) + ".json"), "w", encoding='utf8'))
                                        table_No=table_No+1
                                           
        print('Total table: ', table_folder_count, 'With revision: ', table_count_with_revision, 'Error Table: ', self.table_count_with_error)
        txt_file="table_folder_count: "+ str(table_folder_count)+ ". With Revision: "+ str(table_count_with_revision) + "Error Table with revision: " + str(self.table_count_with_error) + "Unique table error found: " + str(unique_table_error_found)
        with open("table_count_07_09.txt", "w") as text_file:
